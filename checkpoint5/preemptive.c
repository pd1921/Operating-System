...
__data __at (0x39) char visited[MAXTHREADS];
__data __at (0x3D) char vtmp;
__data __at (0x3E) char visitedCount;
/*
 * @@@ [8 pts]
...
void myTimer0Handler(void){
    /* something must wrong here, mabye is about the order of yeild */
    visited[currentThread] = 1;
    visitedCount++;
    if( visitedCount >= threadCount ){
        for( i=0; i<MAXTHREADS; i++ )
            visited[i] = 0;
        visitedCount = 1;
        visited[currentThread] = 1;
